...
function replaceDomElementExtSrc(url, url_src, html, proxy, base64, selector, text_fail = '', selector_source = selector, selector_archive = selector, blocked = false) {
  ...
  if (html) {
    ...
    window.setTimeout(function () {
      ...
      if (article_new) {
        article_new.querySelectorAll('img[src^="data:image/"][currentsourceurl]').forEach(e => e.src = e.getAttribute('currentsourceurl'));
    ...
    if (!url_src && url.includes('/https://www.thetimes.com/')) {
      ...
      if (article_link)
        article_link.before(googleSearchToolLink(url_orig));
    }
  }
}
